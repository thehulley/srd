...
function scEventControl_init(iSeqRow) {
  scEventControl_data["crime_id_" + iSeqRow] = {"blur": false, "change": false, "autocomp": false, "original": "", "calculated": ""};
  scEventControl_data["tempo_" + iSeqRow] = {"blur": false, "change": false, "autocomp": false, "original": "", "calculated": ""};
}

function scEventControl_active(iSeqRow) {
  if (scEventControl_data["crime_id_" + iSeqRow]["blur"]) {
    return true;
  }
  if (scEventControl_data["crime_id_" + iSeqRow]["change"]) {
    return true;
  }
  if (scEventControl_data["tempo_" + iSeqRow]["blur"]) {
    return true;
  }
  if (scEventControl_data["tempo_" + iSeqRow]["change"]) {
    return true;
  }
  return false;
...
function scJQEventsAdd(iSeqRow) {
  $('#id_sc_field_id_' + iSeqRow).bind('change', function() { sc_form_crimes_detento_id__onchange(this, iSeqRow) });
  $('#id_sc_field_detento_id_' + iSeqRow).bind('change', function() { sc_form_crimes_detento_detento_id__onchange(this, iSeqRow) });
  $('#id_sc_field_crime_id_' + iSeqRow).bind('blur', function() { sc_form_crimes_detento_crime_id__onblur(this, iSeqRow) })
                                       .bind('change', function() { sc_form_crimes_detento_crime_id__onchange(this, iSeqRow) })
                                       .bind('focus', function() { sc_form_crimes_detento_crime_id__onfocus(this, iSeqRow) });
  $('#id_sc_field_tempo_' + iSeqRow).bind('blur', function() { sc_form_crimes_detento_tempo__onblur(this, iSeqRow) })
                                    .bind('change', function() { sc_form_crimes_detento_tempo__onchange(this, iSeqRow) })
                                    .bind('focus', function() { sc_form_crimes_detento_tempo__onfocus(this, iSeqRow) });
} // scJQEventsAdd

function sc_form_crimes_detento_id__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
}

function sc_form_crimes_detento_detento_id__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
}

function sc_form_crimes_detento_crime_id__onblur(oThis, iSeqRow) {
  do_ajax_form_crimes_detento_validate_crime_id_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_form_crimes_detento_crime_id__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
  do_ajax_form_crimes_detento_event_crime_id__onchange(iSeqRow);
  nm_check_insert(iSeqRow);
}

function sc_form_crimes_detento_crime_id__onfocus(oThis, iSeqRow) {
  scEventControl_onFocus(oThis, iSeqRow);
  scCssFocus(oThis, iSeqRow);
}

function sc_form_crimes_detento_tempo__onblur(oThis, iSeqRow) {
  do_ajax_form_crimes_detento_validate_tempo_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_form_crimes_detento_tempo__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
  nm_check_insert(iSeqRow);
}

function sc_form_crimes_detento_tempo__onfocus(oThis, iSeqRow) {
  scEventControl_onFocus(oThis, iSeqRow);
  scCssFocus(oThis, iSeqRow);
}
...
function displayChange_block_0(status) {
	displayChange_field("crime_id_", "", status);
	displayChange_field("tempo_", "", status);
}

function displayChange_row(row, status) {
	displayChange_field_crime_id_(row, status);
	displayChange_field_tempo_(row, status);
}
...
function displayChange_field_crime_id_(row, status) {
}

function displayChange_field_tempo_(row, status) {
}
...
function scHidePage(pageNo) {
	$("#id_form_crimes_detento_form" + pageNo).hide();
}
            ...
            if(!$(t).is('img')){
                ...
                switch(field){
                     ...
                     if(usa_read_only && $('a',$('#id_read_on_'+field+iSeqRow)).length == 0){
                         $(t).html("<a href=\"javascript:nm_mostra_doc('0', '"+rs2+"', 'form_crimes_detento')\">"+$('#id_read_on_'+field+iSeqRow).text()+"</a>");
                     }
                }
