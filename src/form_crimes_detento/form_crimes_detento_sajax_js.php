  ...
  function scAjaxUpdateErrors(sType)
  {
    ...
    if (oResp["errList"])
    {
      for (iFieldErrors = 0; iFieldErrors < oResp["errList"].length; iFieldErrors++)
      {
        ...
        if ("geral_form_crimes_detento" == sTestField)
        {
...
//    return document.F1.elements[sFieldName].value.replace(/[+]/g, "__NM_PLUS__");
  ...
  function scMoveMasterDetail(widget)
  {
	...
	function scAjax_displayEmptyForm() {
        ...
        sc_hide_form_crimes_detento_form();
	}
  ...
  // ---------- Validate crime_id_
  function do_ajax_form_crimes_detento_validate_crime_id_(iNumLinha)
  {
    var nomeCampo_crime_id_ = "crime_id_" + iNumLinha;
    var var_crime_id_ = scAjaxGetFieldSelect(nomeCampo_crime_id_);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_form_crimes_detento_validate_crime_id_(var_crime_id_, iNumLinha, var_script_case_init, do_ajax_form_crimes_detento_validate_crime_id__cb);
  } // do_ajax_form_crimes_detento_validate_crime_id_

  function do_ajax_form_crimes_detento_validate_crime_id__cb(sResp)
  {
    ...
    if ("" != iLineNumber)
    {
      sFieldValid = "crime_id_" + iLineNumber;
    }
    else
    {
      sFieldValid = "crime_id_";
    }
    ...
    if ("" == sFieldErrors)
    {
      ...
      scErrorLineOff(iLineNumber, "crime_id_");
    }
    else
    {
      var sImgStatus = sc_img_status_err;
      scAjaxShowErrorDisplay(sFieldValid, sFieldErrors);
      scErrorLineOn(iLineNumber, "crime_id_");
    }
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_form_crimes_detento_validate_crime_id__cb

  // ---------- Validate tempo_
  function do_ajax_form_crimes_detento_validate_tempo_(iNumLinha)
  {
    var nomeCampo_tempo_ = "tempo_" + iNumLinha;
    var var_tempo_ = scAjaxGetFieldHidden(nomeCampo_tempo_);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_form_crimes_detento_validate_tempo_(var_tempo_, iNumLinha, var_script_case_init, do_ajax_form_crimes_detento_validate_tempo__cb);
  } // do_ajax_form_crimes_detento_validate_tempo_

  function do_ajax_form_crimes_detento_validate_tempo__cb(sResp)
  {
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    iLineNumber = scAjaxGetLineNumber();
    if ("" != iLineNumber)
    {
      sFieldValid = "tempo_" + iLineNumber;
    }
    else
    {
      sFieldValid = "tempo_";
    ...
    if ("" == sFieldErrors)
    {
      var sImgStatus = sc_img_status_ok;
      scAjaxHideErrorDisplay(sFieldValid);
      scErrorLineOff(iLineNumber, "tempo_");
    }
    else
    {
      var sImgStatus = sc_img_status_err;
      scAjaxShowErrorDisplay(sFieldValid, sFieldErrors);
      scErrorLineOn(iLineNumber, "tempo_");
    }
    var $oImg = $('#id_sc_status_' + sFieldValid);
    if (0 < $oImg.length)
    {
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_form_crimes_detento_validate_tempo__cb

  // ---------- Event onchange crime_id_
  function do_ajax_form_crimes_detento_event_crime_id__onchange(iSeqForm)
  {
    var var_tempo_ = scAjaxGetFieldHidden("tempo_" + iSeqForm);
    var var_crime_id_ = scAjaxGetFieldSelect("crime_id_" + iSeqForm);
    var var_script_case_init = document.F2.script_case_init.value;
    var var_nmgp_refresh_row = iSeqForm;
    scAjaxProcOn(true);
    x_ajax_form_crimes_detento_event_crime_id__onchange(var_tempo_, var_crime_id_, var_script_case_init, var_nmgp_refresh_row, do_ajax_form_crimes_detento_event_crime_id__onchange_cb);
  } // do_ajax_form_crimes_detento_event_crime_id__onchange

  function do_ajax_form_crimes_detento_event_crime_id__onchange_cb(sResp)
  {
    scAjaxProcOff(true);
    oResp = scAjaxResponse(sResp);
    iLineNumber = scAjaxGetLineNumber();
    ...
    scAjaxSetReadonly();
    scAjaxSetMaster();
    scAjaxAlert(do_ajax_form_crimes_detento_event_crime_id__onchange_cb_after_alert);
  } // do_ajax_form_crimes_detento_event_crime_id__onchange_cb
  function do_ajax_form_crimes_detento_event_crime_id__onchange_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
    scAjaxRedir();
  } // do_ajax_form_crimes_detento_event_crime_id__onchange_cb_after_alert

  var sc_num_ult_line = "";
  var sc_insert_open  = false;

  // ---------- add_new_line
  function do_ajax_form_crimes_detento_add_new_line(sc_clone, sc_seq_clone)
  {
    if (sc_insert_open)
    {
        if (sc_clone == 'S' && sc_seq_clone != iAjaxNewLine)
        {
          do_ajax_form_crimes_detento_cancel_insert(iAjaxNewLine);
        }
        else
        {
          return;
    ...
    var var_script_case_init = document.F1.script_case_init.value;
    scAjaxProcOn(true);
    x_ajax_form_crimes_detento_add_new_line(var_sc_clone, var_sc_seq_clone, var_sc_seq_vert, var_script_case_init, do_ajax_form_crimes_detento_add_new_line_cb);
  } // do_ajax_form_crimes_detento_add_new_line

  function do_ajax_form_crimes_detento_add_new_line_cb(sResp)
  {
    scAjaxProcOff(true);
    if ("{" == sResp.substr(0, 1)) {
        oResp = scAjaxResponse(sResp);
  ...
  } // do_ajax_form_crimes_detento_add_new_line_cb

  // ---------- backup_line
  function do_ajax_form_crimes_detento_backup_line(iNumLinha)
  {
    var var_id_ = scAjaxGetFieldText("id_" + iNumLinha);
    var var_nmgp_refresh_row = iNumLinha;
    var var_nm_form_submit = document.F1.nm_form_submit.value;
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_form_crimes_detento_backup_line(var_id_, var_nmgp_refresh_row, var_nm_form_submit, var_script_case_init, do_ajax_form_crimes_detento_backup_line_cb);
  } // do_ajax_form_crimes_detento_backup_line

  function do_ajax_form_crimes_detento_backup_line_cb(sResp)
  {
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    if (!scAjaxHasError())
  ...
  } // do_ajax_form_crimes_detento_backup_line_cb

  function do_ajax_form_crimes_detento_cancel_insert(iSeqVert)
  {
    var oTBodyOld = document.getElementById("hidden_bloco_0").tBodies[0];
  ...
  } // do_ajax_form_crimes_detento_cancel_insert

  function do_ajax_form_crimes_detento_cancel_update(iSeqVert)
  {
    do_ajax_form_crimes_detento_backup_line(iSeqVert);
...
?>
  } // do_ajax_form_crimes_detento_cancel_update

  function do_ajax_form_crimes_detento_restore_buttons()
  {
<?php
...
?>
  } // do_ajax_form_crimes_detento_restore_buttons

  // ---------- table_refresh
  function do_ajax_form_crimes_detento_table_refresh()
  {
    ...
    var var_script_case_init = document.F2.script_case_init.value;
    scAjaxProcOn();
    x_ajax_form_crimes_detento_table_refresh(var_id_, var_nm_form_submit, var_nmgp_opcao, var_nmgp_ordem, var_nmgp_arg_dyn_search, var_script_case_init, do_ajax_form_crimes_detento_table_refresh_cb);
  } //  do_ajax_form_crimes_detento_table_refresh

  function do_ajax_form_crimes_detento_table_refresh_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    ...
    scAjaxAlert(do_ajax_form_crimes_detento_table_refresh_cb_after_alert);
  } // do_ajax_form_crimes_detento_table_refresh_cb
  function do_ajax_form_crimes_detento_table_refresh_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
    ...
    sc_insert_open = false;
  } // do_ajax_form_crimes_detento_table_refresh_cb_after_alert
...
function scJs_sweetalert_params(params) {
  ...
  var sc_num_ult_opc  = "";
  var sc_num_ult_tr   = "";
  function do_ajax_form_crimes_detento_submit_form(iNumLinha, indexTR)
  {
    if (scEventControl_active(iNumLinha)) {
      setTimeout(function() { do_ajax_form_crimes_detento_submit_form(iNumLinha, indexTR); }, 500);
      return;
    }
    sc_num_ult_line = iNumLinha;
    sc_num_ult_tr   = indexTR;
    scAjaxHideMessage();
    var var_crime_id_ = scAjaxGetFieldSelect("crime_id_" + iNumLinha);
    var var_tempo_ = scAjaxGetFieldHidden("tempo_" + iNumLinha);
    var var_nm_form_submit = document.F1.nm_form_submit.value;
...
?>
    x_ajax_form_crimes_detento_submit_form(var_crime_id_, var_tempo_, var_nmgp_refresh_row, var_nm_form_submit, var_nmgp_url_saida, var_nmgp_opcao, var_nmgp_ancora, var_nmgp_num_form, var_nmgp_parms, var_script_case_init, var_csrf_token, do_ajax_form_crimes_detento_submit_form_cb);
  } // do_ajax_form_crimes_detento_submit_form

  function do_ajax_form_crimes_detento_submit_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
...
?>
      ...
      scAjaxHideErrorDisplay("table");
      scAjaxHideErrorDisplay("crime_id_" + sc_num_ult_line);
      scAjaxHideErrorDisplay("tempo_" + sc_num_ult_line);
<?php
if (isset($this->Embutida_form) && $this->Embutida_form) {
?>
      scAjaxSetReadonly();
...
?>
    ...
    scAjaxSetLabel(true);
    scAjaxSetMaster();
    scAjaxAlert(do_ajax_form_crimes_detento_submit_form_cb_after_alert);
  } // do_ajax_form_crimes_detento_submit_form_cb
  function do_ajax_form_crimes_detento_submit_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
    scAjaxRedir();
  ...
  } // do_ajax_form_crimes_detento_submit_form_cb_after_alert

  var scStatusDetail = {};

  function do_ajax_form_crimes_detento_navigate_form()
  {
    if (scFormHasChanged) {
      scJs_confirm('<?php echo html_entity_decode($this->Ini->Nm_lang['lang_reload_confirm']) ?>', perform_ajax_form_crimes_detento_navigate_form, function() {});
    } else {
      perform_ajax_form_crimes_detento_navigate_form();
    }
  }

  function perform_ajax_form_crimes_detento_navigate_form()
  {
    if (scRefreshTable())
    {
      return;
    ...
    if (sc_insert_open)
    {
        do_ajax_form_crimes_detento_cancel_insert(sc_num_ult_line);
    }
    nm_uncheck_delete();
    scAjaxHideMessage();
    scAjaxHideErrorDisplay("table");
    ...
    {
      scAjaxHideErrorDisplay("crime_id_" + iNavForm);
      scAjaxHideErrorDisplay("tempo_" + iNavForm);
    }
    var var_id_ = document.F2.id_.value;
    var var_nm_form_submit = document.F2.nm_form_submit.value;
    var var_nmgp_opcao = document.F2.nmgp_opcao.value;
    ...
    var var_script_case_init = document.F2.script_case_init.value;
    scAjaxProcOn();
    x_ajax_form_crimes_detento_navigate_form(var_id_, var_nm_form_submit, var_nmgp_opcao, var_nmgp_ordem, var_nmgp_arg_dyn_search, var_script_case_init, do_ajax_form_crimes_detento_navigate_form_cb);
  } // do_ajax_form_crimes_detento_navigate_form

  var scMasterDetailParentIframe = "<?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['form_crimes_detento']['dashboard_info']['parent_widget'] ?>";
...
?>
  function do_ajax_form_crimes_detento_navigate_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    ...
    scErrorLineReset();
    $('.sc-js-ui-statusimg').css('display', 'none');
    scAjaxAlert(do_ajax_form_crimes_detento_navigate_form_cb_after_alert);
  } // do_ajax_form_crimes_detento_navigate_form_cb
  function do_ajax_form_crimes_detento_navigate_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
<?php
...
?>
  ...
  } // do_ajax_form_crimes_detento_navigate_form_cb_after_alert
  function sc_hide_form_crimes_detento_form()
  {
...
?>
  ...
  {
    ajax_field_list[iTotCampos] = "crime_id_" + iNumLinha;
    iTotCampos++;
    ajax_field_list[iTotCampos] = "tempo_" + iNumLinha;
    iTotCampos++;
    ajax_error_list["crime_id_" + iNumLinha] = {"label": "Crime<?php echo $sLineInfo; ?>", "valid": new Array(), "onblur": new Array(), "onchange": new Array(), "onclick": new Array(), "onfocus": new Array(), "timeout": 5};
    ajax_error_list["tempo_" + iNumLinha] = {"label": "Tempo<?php echo $sLineInfo; ?>", "valid": new Array(), "onblur": new Array(), "onchange": new Array(), "onclick": new Array(), "onfocus": new Array(), "timeout": 5};
    ajax_field_mult["crime_id_"][iNumLinha] = "crime_id_" + iNumLinha;
    ajax_field_mult["tempo_"][iNumLinha] = "tempo_" + iNumLinha;
    ajax_field_id["crime_id_" + iNumLinha] = new Array("hidden_field_label_crime_id_", "hidden_field_data_crime_id_" + iNumLinha);
    ajax_field_id["tempo_" + iNumLinha] = new Array("hidden_field_label_tempo_", "hidden_field_data_tempo_" + iNumLinha);
    ajax_error_count["crime_id_" + iNumLinha] = "off";
    ajax_error_count["tempo_" + iNumLinha] = "off";
<?php
if (!$this->Grid_editavel)
{
?>
    ajax_read_only["crime_id_" + iNumLinha] = "off";
    ajax_read_only["tempo_" + iNumLinha] = "off";
<?php
}
else
{
?>
    ajax_read_only["crime_id_" + iNumLinha] = "on";
    ajax_read_only["tempo_" + iNumLinha] = "on";
<?php
}
?>
  }
  ...
  {
    ajax_error_list["crime_id_" + iNumLinha] = null;
    ajax_error_list["tempo_" + iNumLinha] = null;
  }
  ...
  function scRemoveErrors()
  {
    for (iNumLinha = 1; iNumLinha < Lim_linhas; iNumLinha++)
    {
      ajax_error_list["crime_id_" + iNumLinha]["valid"] = new Array();
      ajax_error_list["crime_id_" + iNumLinha]["onblur"] = new Array();
      ajax_error_list["crime_id_" + iNumLinha]["onchange"] = new Array();
      ajax_error_list["crime_id_" + iNumLinha]["onclick"] = new Array();
      ajax_error_list["crime_id_" + iNumLinha]["onfocus"] = new Array();
      ajax_error_list["tempo_" + iNumLinha]["valid"] = new Array();
      ajax_error_list["tempo_" + iNumLinha]["onblur"] = new Array();
      ajax_error_list["tempo_" + iNumLinha]["onchange"] = new Array();
      ajax_error_list["tempo_" + iNumLinha]["onclick"] = new Array();
      ajax_error_list["tempo_" + iNumLinha]["onfocus"] = new Array();
    }
  }
...
?>
  ...
  {
<?php
  $NM_contr_readonly = (isset($this->nmgp_cmp_readonly['crime_id_'])) ? $this->nmgp_cmp_readonly['crime_id_'] : 'off';
?>
    scAjaxFieldRead("crime_id_" + iSeq, "<?php echo $NM_contr_readonly ?>");
<?php
  $NM_contr_readonly = (isset($this->nmgp_cmp_readonly['tempo_'])) ? $this->nmgp_cmp_readonly['tempo_'] : 'off';
?>
    scAjaxFieldRead("tempo_" + iSeq, "<?php echo $NM_contr_readonly ?>");
  }

  function mdCloseObjects(iSeq)
  {
    scAjaxFieldRead("crime_id_" + iSeq, "on");
    scAjaxFieldRead("tempo_" + iSeq, "on");
    rerunHeaderDisplay = 1;
    scSetFixedHeaders(true);
  }
...
?>
  ...
  function scErrorLineOn(iRow, sIdError)
  {
    ...
    if (bErrorRow || ("on" == ajax_error_count["crime_id_" + iRow] || "on" == ajax_error_count["tempo_" + iRow]))
    {
      $("#hidden_field_data_sc_seq" + iRow).addClass("scFormErrorLine");
      $("#hidden_field_data_sc_actions" + iRow).addClass("scFormErrorLine");
      $("#hidden_field_data_crime_id_" + iRow).addClass("scFormErrorLine");
      $("#hidden_field_data_tempo_" + iRow).addClass("scFormErrorLine");
    }
  }
  ...
  function scErrorLineOff(iRow, sIdError)
  {
    ...
    if (bErrorRow || ("off" == ajax_error_count["crime_id_" + iRow] && "off" == ajax_error_count["tempo_" + iRow]))
    {
      if (bErrorRow)
      {
        ajax_error_count["crime_id_" + iRow] = "off";
        ajax_error_count["tempo_" + iRow] = "off";
      }
      ...
      $("#hidden_field_data_crime_id_" + iRow).removeClass("scFormErrorLine");
      $("#hidden_field_data_tempo_" + iRow).removeClass("scFormErrorLine");
    }
  }
