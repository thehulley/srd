...
//
      ...
      if ($Campos_Crit != "" || !empty($Campos_Falta) || $this->Campos_Mens_erro != "" || $campos_erro != "" || !isset($this->bok) || $this->bok != "OK" || $this->nmgp_opcao == "recarga")
      {
          if ($Campos_Crit == "" && empty($Campos_Falta) && $this->Campos_Mens_erro == "" && !isset($this->bok) && $this->nmgp_opcao != "recarga")
          {
              ...
              { 
                  ...
                  $remember_me = $_SESSION['sc_session'][$this->Ini->sc_page]['app_Login']['campos'][3]; 
              } 
          }
          $this->nm_gera_html();
          $this->NM_close_db(); 
      ...
      elseif (isset($this->bok) && $this->bok == "OK")
      {
          ...
          $_SESSION['sc_session'][$this->Ini->sc_page]['app_Login']['campos'][3] = $this->remember_me; 
          if (!empty($this->links))
          {
              ...
              else
              {
                  ...
                  $_SESSION['sc_session'][$this->Ini->sc_page]['app_Login']['redir_target_name'] = $this->links_sc_target_name;
...
if (!isset($this->sc_temp_usr_login)) {$this->sc_temp_usr_login = (isset($_SESSION['usr_login'])) ? $_SESSION['usr_login'] : "";}
	...
	$sql = "INSERT INTO
				funcionario( matricula, nome, senha, email, ativo )
			VALUES 
				(". $this->Db->qstr($usr_id). ", ".
					$this->Db->qstr($name) . ", ".
	...
	$sql = "INSERT INTO
				funcionario_social( funcionario_social.login, funcionario_social.resource, funcionario_social.resource_id )
			VALUES 
				(". $this->Db->qstr($usr_id). ", ".
...
if(isset($_COOKIE['usr_data']) && !empty($_COOKIE['usr_data'])){
    ...
    if(substr($usr_data['code'], 0, 7) == 'cookie_'){
        ...
        $sql = "SELECT 
                    ...
                    email 
                FROM funcionario 
                WHERE matricula = ". $this->Db->qstr($usr_data['login']) ."
...
if($this->remember_me  == 1){
    ...
    $slogin = $this->Db->qstr($this->login );
     ...
     $nm_select ="UPDATE funcionario SET codigo_ativacao = ". $this->Db->qstr($code) . " WHERE login = ". $slogin; 
         $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_select;
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = ''; 
         $rf = $this->Db->Execute($nm_select);
         if ($rf === false)
		...
		impressao
	      ...
	      WHERE grupo_id IN
		   ...
		   FROM
		       grupo_usuario 
		   WHERE
		       funcionario = '". $this->sc_temp_usr_login ."')";
